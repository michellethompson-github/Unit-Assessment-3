...
class Character {
  ...
  getInfo() {
    // `${this.name} is goodp erson`
    // this.name+"is a good person"
    return `This is a ${this.name} character named ${this.type}.`;
  }
}
let myCharacter=new Character("darshan","decent")
console.log(myCharacter.getInfo())
//////////////////PROBLEM 2////////////////////
...
let myNpc = new NPC("darshan","decnd","london","is a beautiful")

console.log(myNpc.dialogue())
/*
...
//CODE HERE
let myCharacter = new NPC(
  "Ralph",
  "Decent",
  "who is a human located in Niceland",
  "I'm gonna wreck it!"
);
/*
...
let ralphsInfo = myCharacter.getInfo();
let ralphsDialogue = myCharacter.dialogue();
let ralphsLocation = myCharacter.location;
...
class Player extends Character {
  ...
  defend(amount) {
    let tempAttackLevel = this.attackLevel - amount;
    if (tempAttackLevel > 0) {
      return {
        attackStrength: amount,
        remainingHealth: tempAttackLevel,
...
//CODE HERE
const aang = new Player("Aang", "airbender", 100);
...
class Hero extends Player {
  ...
  useSuperPower(index) {
    // let a=[3,4,5,6,6]
    let tempPower = this.superPowers[index];
    return `${this.name} used ${tempPower}`;
  }
}
//CODE HERE
...
fireSpitter.addSuperPower("Fight");
